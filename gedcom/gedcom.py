...
class Entry:
    ...
    def get_multi_chain(self, chain):
        """
        Like get_chain, but retrieves all the matching values instead of only
    ...
    def get_chain(self, chain):
        """
        Retrieves an object or value further from the hierarchy. The chain
        describes the path to the value with objects in hierarcy separated with
        dots. The chain can contain gedcom tag names, one of the keywords in if
        or a data key generated by some additional processing. For example with
        BIRT.DATE.value we load BIRT gedcom entry from inside the calling
        entry, DATE entry from inside BIRT and take the value of that entry.
        """
        tag = self
